    ...
    <section className="bg-slate-50 py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {staff.map(agent => (
            <div key={agent.name} className={`bg-white rounded-2xl p-6 border hover:shadow-lg transition-all ${agent.premium ? 'border-violet-200 relative' : 'border-slate-200 hover:border-slate-300'}`}>
              {agent.premium && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-violet-600 text-white text-[10px] font-bold px-3 py-1 rounded-full">PREMIUM</span>}
