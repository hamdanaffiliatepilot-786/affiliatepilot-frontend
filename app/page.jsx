...
export default function Home() {
  ...
  // VIRAL TOOL: Instagram Reel Product Finder
  const findReelProduct = async (e) => {
    e.preventDefault();
    const link = e.target.reelLink.value;
    setReelResult("🤖 AI is analyzing the Reel video... Detecting product...");
    await new Promise(resolve => setTimeout(resolve, 2000)); // Simulating AI processing
    setReelResult(`✅ Product Found in Reel!\n\n📌 Product: "Trendy Wireless Earbuds"\n\n💰 Best Price Found: ₹1,299\n\n🛒 Buy on Amazon: https://www.amazon.in/s?k=wireless+earbuds\n\n🛒 Buy on Flipkart: https://www.flipkart.com/search?q=wireless+earbuds\n\n*(Note: Real-time Reel scraping requires Instagram API approval. Currently showing simulated AI results.)*`);
  };
    ...
    <>
      ...
      <main className="min-h-screen bg-gray-50 text-gray-900 font-sans">
        ...
        <section className="bg-gradient-to-br from-slate-900 to-indigo-950 text-white py-20 text-center px-4">
          ...
          <p className="text-lg text-gray-300 mb-4">Real-time currency, EMI calculator, AI deal finder & Reel product finder.</p>
        ...
        <section id="tools" className="max-w-7xl mx-auto p-6 grid md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8">
          ...
          {/* 1. VIRAL TOOL: Instagram Reel Product Finder */}
          <div className="bg-gradient-to-br from-pink-50 to-purple-50 p-6 rounded-2xl shadow-sm border border-pink-100 md:col-span-2 lg:col-span-2">
            <h3 className="font-bold text-xl mb-2">🎬 Instagram Reel Product Finder</h3>
            ...
            <form onSubmit={findReelProduct} className="space-y-3">
              <input name="reelLink" type="url" placeholder="https://www.instagram.com/reel/..." className="w-full border p-2 rounded-lg" required />
              <button type="submit" className="w-full bg-gradient-to-r from-pink-500 to-purple-500 text-white p-2 rounded-lg font-bold hover:from-pink-600 hover:to-purple-600">Find Product</button>
            </form>
            <p className="mt-3 text-sm bg-white p-3 rounded-lg whitespace-pre-wrap h-40 overflow-auto">{reelResult}</p>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border">
            ...
            <p className="mt-3 text-sm bg-blue-50 p-2 rounded-lg whitespace-pre-wrap h-28 overflow-auto">{dealResult}</p>
          </div>
        ...
        <section id="store" className="py-20 bg-white max-w-7xl mx-auto px-4">
          ...
          {products.length === 0 ? (
            <div className="text-center bg-gray-50 p-10 rounded-2xl border shadow-sm">
              <p className="text-gray-500 font-medium">⏳ Waiting for system to sync products...</p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {products.map(p => (
                <div key={p.id} className="bg-gray-50 rounded-2xl overflow-hidden shadow-sm border hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
